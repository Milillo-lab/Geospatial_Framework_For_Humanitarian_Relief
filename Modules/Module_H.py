...
# --- Origin/Destination (choose either node IDs or lon/lat) ---
...
# How many distinct shortest routes to collect (1..10)
ROUTE_TARGET = require("ROUTE_TARGET", int)

# --- Output base (consistent with previous sections) ---
OUT_BASE   = r"./Outputs"
...
def ensure_nodes_index_xy(nodes_gdf):
    n = nodes_gdf.copy()
...
# ========================= H3) Load Inputs =========================
# Nodes: memory first, else Outputs_Final\A
...
# Evaluated edges (with TB override): memory first, else Outputs_Final\F\E
...
# Resolve O/D nodes (ID first; else nearest by lon/lat)
...
# =============== H6) Export CSV + PNGs (smaller images) ===============
...
P = {
    ...
    "lon_min": 36.850,                          # min longitude (None = auto extent)
    "lon_max": 36.950,                          # max longitude (None = auto extent)
    "lat_min": 37.550,                          # min latitude  (None = auto extent)
    "lat_max": 37.600,                          # max latitude  (None = auto extent)
    ...
    # ---- Scalebar (assumes ScaleBar was imported earlier in Section H) ----
...
def _plot_route_png(i, r):
    ...
    # Scalebar (assumes ScaleBar was imported in section header)
    if P["ADD_SCALEBAR"]:
