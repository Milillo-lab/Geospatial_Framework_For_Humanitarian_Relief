...
OUT_BLDG_ALL   = "all_buildings_selected_region.shp"      # all buildings (no height filter)
OUT_BLDG_CLEAN = "cleaned_buildings_selected_region.shp"  # after height filter
...
# === A4) WFS download using AOI bbox → clip to AOI polygon ==========
...
print(f"  Buildings inside AOI after clip: {len(bld_3857)}")

# ========= A5) Save all, apply height threshold, compute centroids, prepare for plotting ==========

# 1) Save ALL buildings before height filtering
bldg_all_path = os.path.join(OUT_DIR, OUT_BLDG_ALL)
bld_3857.to_file(bldg_all_path)
print(f"\nExported ALL AOI-clipped buildings (pre-filter) → {bldg_all_path}")
print(f"  Total buildings saved (ALL, inside AOI): {len(bld_3857)}")

# 2) Apply height threshold filter
print("\nApplying height threshold filter to buildings ...")

HEIGHT_THRESHOLD = require("HEIGHT_THRESHOLD", float)
HEIGHT_COL       = require("HEIGHT_COL", str)

if HEIGHT_COL not in bld_3857.columns:
    print(f"  WARNING: height column '{HEIGHT_COL}' not found in building data.")
    print("  Skipping height-based filtering (all buildings kept).")
else:
    # Convert height column to numeric (invalid values → NaN)
    bld_3857[HEIGHT_COL] = pd.to_numeric(bld_3857[HEIGHT_COL], errors="coerce")

    before_n = len(bld_3857)
    h = bld_3857[HEIGHT_COL]

    # Keep only buildings strictly higher than threshold
    mask_keep = h > HEIGHT_THRESHOLD
    kept_n = int(mask_keep.sum())
    removed_n = int(before_n - kept_n)

    bld_3857 = bld_3857.loc[mask_keep].copy()

    print(f"  Height column                        : '{HEIGHT_COL}'")
    print(f"  Height threshold (m)                 : {HEIGHT_THRESHOLD:.2f}")
    print(f"  Buildings before height filter       : {before_n}")
    print(f"  Buildings removed (<= thr / NaN)     : {removed_n}")
    print(f"  Buildings kept (> threshold)         : {kept_n}")

# 3) Centroid calculation for CLEANED buildings
print("  Computing centroids for CLEANED buildings ...")
...
# 4) Save CLEANED buildings shapefile into Outputs\A
bldg_clean_path = os.path.join(OUT_DIR, OUT_BLDG_CLEAN)
bld_3857.to_file(bldg_clean_path)
print(f"\nExported CLEANED AOI-clipped buildings (post-filter) → {bldg_clean_path}")
print(f"  Total buildings saved (CLEANED, inside AOI): {len(bld_3857)}")

# 5) Prepare buildings_gdf for A6 plotting
buildings_gdf = bld_3857.copy()
print("  buildings_gdf is now defined for A6 plotting.")
# ========= Prepare datasets dictionary for A6 plotting =========

datasets = {
    "OSM_Edges": gpd.read_file(os.path.join(OUT_DIR, OUT_EDGES)) if os.path.exists(os.path.join(OUT_DIR, OUT_EDGES)) else None,
    "OSM_Nodes": gpd.read_file(os.path.join(OUT_DIR, OUT_NODES)) if os.path.exists(os.path.join(OUT_DIR, OUT_NODES)) else None,
    "Region_Boundary": aoi_gdf_ll.copy() if 'aoi_gdf_ll' in globals() else None
}

print("  datasets{} dictionary created for A6 plotting.")
...
print("\n===== Section A Summary =====")
print(f"Buildings RAW (bld_3857_raw):        {_count('bld_3857_raw')}")
print(f"Buildings clipped (bld_3857):        {_count('bld_3857')}")
print(f"Buildings CLEANED (buildings_gdf):   {_count('buildings_gdf')}")
print(f"OSM Edges (edges):                   {_count('edges')}")
print(f"OSM Nodes (nodes):                   {_count('nodes')}")
print("==========================================\n")
