...
# ============== A1) Inputs, Outputs, and User Parameters ===================
...
# Historical OSM snapshot date (optional in your YAML; here we require it to keep behavior identical)
target_dt = require("target_dt", str)
ox.settings.overpass_settings   = f'[out:json][timeout:180][date:"{target_dt}"]'
ox.settings.overpass_rate_limit = True

# WFS server/layer (Global 3D buildings)
...
OUT_CITY_POLY = "region_boundary.geojson"   # saved when MODE == "place" (or available for others)

# Grid splitting for WFS (helps avoid timeouts)
...
# If MODE == "shapefile": path to a polygon shapefile (inside INPUTS_DIR)
SHP_PATH = os.path.join(INPUTS_DIR, require("SHP_NAME", str))
...
# ===== A3) OSM download (nodes, edges, region boundary if place) ===========
...
if MODE == "place":
    ...
    # AOI polygon for later (we will use its bbox for WFS)
...
# Save OSM nodes and edges into Outputs_Final\A
...
# Download per grid cell via BBOX (EPSG:3857)
all_chunks = []
for i, row in sub_polys_3857.iterrows():
    part_id = i + 1
    cell_bounds = tuple(row.geometry.bounds)  # (minx, miny, maxx, maxy) in meters
    ...
    # Clip strictly to the exact cell polygon (safety)
...
# Merge and clip to the full AOI polygon (strict final cut)
print("\nMerging WFS parts ...")
bld_3857_raw = gpd.GeoDataFrame(pd.concat(all_chunks, ignore_index=True), crs="EPSG:3857")
...
bld_ll = bld_3857.to_crs("EPSG:4326")
bld_3857["longitude"] = bld_ll.geometry.centroid.x
bld_3857["latitude"]  = bld_ll.geometry.centroid.y

# Save buildings shapefile (strictly inside AOI) into Outputs_Final\A
...
P = {
    ...
    "TITLE": "AOI, Edges + Buildings",  # plot title
    ...
    "NA_Y": 0.18,                       # arrow y-position (axes fraction 0–1)
    "NA_LEN": 0.10,                     # arrow length (axes fraction)
    ...
    # ===== Scalebar =====
    "ADD_SCALEBAR": True,               # show scalebar (assumes imported earlier in Section A)
...
# --- minimal helpers (no safety nets) ---
...
# --- choose a CRS source (first requested layer that exists) ---
cand = None
