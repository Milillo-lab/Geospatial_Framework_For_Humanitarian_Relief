...
# ============== B1) Inputs, Outputs, and User Parameters ===================
...
# Inputs
DPM_PATH = Path(os.path.join(INPUTS_DIR, require("DPM_FILE", str)))

# Outputs (vectors & rasters)
...
# Buildings (already AOI-limited in A) — prefer memory
...
with rio.open(DPM_PATH) as src:
    ...
    aoi_geom = [AOI_RCRS.union_all().__geo_interface__]  # shapely 2.x
...
# ========== B4) Threshold → damaged mask GeoTIFF → (optional) polygons =========
...
# Optional: vectorize damaged pixels for plotting
dam_polys = [
    shape(geom)
...
# Save (same variable names expected by downstream code)
...
P = {
    ...
    # ---- Scalebar (assumes ScaleBar is imported at section header) ----
...
# ========== B7) PLOT: DAMAGED (RED) vs UNDAMAGED (GRAY) (LOCAL UTM) =========
# ------------------------- USER CONTROLS (edit here) -------------------------
P = {
    # ---- Output ----
    ...
    # ---- Scalebar (assumes ScaleBar imported in section header) ----
    "ADD_SCALEBAR": True,            # draw scalebar
    "SB_UNITS": "m",                 # scalebar units
    "SB_LOC": "lower right",         # scalebar location
    "SB_BOX_ALPHA": 0.8,             # scalebar box transparency
...
# Datasets (prefer memory)
_b  = buildings_r
_bd = damaged_only
_bu = undamaged_only

# OSM nodes/edges (prefer memory; else load from A outputs if present)
...
# Damaged pixels (binary mask GeoTIFF)
dam_pixels = 0
