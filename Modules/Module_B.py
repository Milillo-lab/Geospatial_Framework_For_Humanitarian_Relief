...
DAMAGED_MASK_TIF  = Path(os.path.join(OUT_DIR, "AOI_DPM_DamagedMask.tif"))
...
if 'bld_3857' in globals() and bld_3857 is not None and not bld_3857.empty:
    ...
    buildings_path = Path(os.path.join(A_OUT_DIR, "cleaned_buildings_selected_region.shp"))
    if not buildings_path.exists():
        raise FileNotFoundError("Buildings not in memory and cleaned_buildings_selected_region.shp not found in Outputs_Final\\A.")
...
# IMPORTANT: reset index so we can safely index damaged_mask_buildings by .index
buildings_r = buildings_r.reset_index(drop=True)
...
def _vectorize_damaged_in_bounds(bounds):
    ...
    with rio.open(tif_clip) as ds:
        ...
        if DISSOLVE_IN_TILE:
            ...
            if getattr(u, "geom_type", "") == "MultiPolygon":
                geoms = list(u.geoms)
            else:
                geoms = [u]
        else:
...
for idx, tb in enumerate(tiles, start=1):
    ...
    try:
        # Query buildings that intersect this tile bbox
    ...
    if hit_count > 0:
        ...
        # cand.index is now 0..N-1 because we reset_index on buildings_r
...
P = {
    ...
    "TITLE": "Buildings + Damaged Pixels ",
    "XLABEL": "Easting (m)",
    "YLABEL": "Northing (m)",
...
P = {
    ...
    "DPI": 300,                      # export resolution
    "TITLE": "Buildings: Damaged vs Undamaged",
    "XLABEL": "Easting (m)",
    "YLABEL": "Northing (m)",
